...
export default function onClick(event) {
    ...
    const Perso = intersects.find(intersect => intersect.object.name.toLowerCase() === "куб_02");
        ...
        if (previousTimeStamp !== timestamp) {
            ...
            if (Perso) {
                ...
                if (!done) {
                    ...
                    if (elapsed > passeSousLeSol && elapsed < finAnimation - 400) {
                        camera.position.y -= 0.1;
                    }
                    if(elapsed > 2700 && elapsed < finAnimation - 500){
                        camera.zoom += 0.16;
